...
const url = "https://www.nature.com/news";

// Functions
function isValidLink(link) {
    return link.includes("https://") || link.includes("/articles");
};

// Exports
module.exports = async (interaction, toReCheck) => {
    await zpravy_service.handleMainFunction(interaction, toReCheck, url,
             "veda", "nature", "a[href*=\"/articles/\"]", isValidLink, MAX_TITLES, "nature");
    /*let channel;
    if(interaction === global.discordClient) {
        interaction = toReCheck;
        toReCheck = [];

        channel = interaction.channels.cache.get(channels_Zpravy["veda"]);
    } else {
        interaction.reply({ content: 'Secret Pong!', flags: MessageFlags.Ephemeral });

        channel = await interaction.member.guild.channels.cache.get(channels_Zpravy["veda"]);
    }
    if(!channel) return;
    const url = "https://www.nature.com/news";

    let arrayQueryToCheck = await postgres_service.runQuery("SELECT * FROM zpravy WHERE redakce='nature'");
    arrayQueryToCheck.rows[0] ??= { url: "null" };

    scraperjs.StaticScraper.create(url).scrape(function ($) {
        return $("a[href*=\"/articles/\"]").map(function() {
        ...
        for(const link of uniq) {
            if(!link.includes("https://") || !link.includes("/articles")) continue;
            const fullLink = link.includes("https://") ? link : "https://www.nature.com" + link;
        ...
        zpravy_service.update_tables_url_toReCheck("nature", toReCheck);
    });*/
}